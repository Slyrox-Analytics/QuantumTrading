...
def theme(fig):
    fig.update_layout(
        ...
        title_font_color="#00ffd0",
        margin=dict(l=10,r=10,t=40,b=10)
    )
    fig.update_xaxes(gridcolor="rgba(0,255,200,0.15)")
    fig.update_yaxes(gridcolor="rgba(0,255,200,0.15)")
    return fig
...
if page=="Dashboard":
    ...
    with c5:st.markdown(f'<div class="card"><h3>Avg ROI</h3><h2>{roi:.2f}%</h2></div>',True)
    ...
    if not df.empty:
        ...
        # Wins vs Losses
        pie=px.pie(names=["Wins","Losses"],values=[wins,losses],hole=0.65,title="Win Ratio")
        st.plotly_chart(theme(pie),use_container_width=True)
        ...
        if not fdf.empty:
            ...
            st.plotly_chart(theme(area),use_container_width=True)
...
# =====================================================
    ...
    with c1: side=st.selectbox("Side",["Long","Short"])
    with c2: margin=st.number_input("Margin",step=1.0)
    with c3: pnl_value=st.number_input("PnL",step=0.1)
